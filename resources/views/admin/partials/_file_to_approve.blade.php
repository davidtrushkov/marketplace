<div class="col-sm-12 no-padding YOUR-FILE-BOX">
    ...
    <form action="{{ route('admin.files.new.update', $file) }}" method="post" id="approve-{{ $file->id }}" class="hidden">
        {{ csrf_field() }}
        {{ method_field('PATCH') }}
    </form>
    ...
    <form action="{{ route('admin.files.new.destroy', $file) }}" method="post" id="reject-{{ $file->id }}" class="hidden">
        {{ csrf_field() }}
        {{ method_field('DELETE') }}
    </form>
</div>
